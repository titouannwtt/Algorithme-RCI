...
def launch_backtest():
    ...
    print(f"Avancement total d'execution : {i}/{count} ({pourcent}%) (Temps restant estimé par rapport aux dernières executions : {((count-i)*elapsed)} secondes soit {(((count-i)*elapsed)/60)} minutes ou {round((count-i)*elapsed/60/60, 1)} heures ou {round((count-i)*elapsed/60/60/24, 1)} jours)")
